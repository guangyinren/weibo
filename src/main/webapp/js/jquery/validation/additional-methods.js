...
$.validator.addMethod("realName", function (value, element, param) {
    var match=false;
    if((/^[\u4e00-\u9fa5]{2,8}$/.test(value))||(/^([A-Za-z]+\s?)*[A-Za-z]$/.test(value)&&value.length>=2&&value.length<=24)){
    	match=true;
    }
    return match;
},"真实姓名只允许汉字2~8位、英文2~24位");
//邮箱校验
$.validator.addMethod("customEmail", function (value, element, param) {
	return /^([.a-zA-Z0-9_-])+@[a-zA-Z0-9_-]{2,}(.[a-zA-Z0-9_-]{2,})+$/.test(value);
},"邮箱格式不正确");
//密码校验
	$.validator.addMethod("customPassword", function (value, element, param) {
