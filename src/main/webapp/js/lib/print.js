...
var PrintJS = function () {
  ...
  switch (typeof args[0]) {
    ...
    case 'object':
      ...
      print.params.pid = args[0].pid || defaultParams.pid;
      break;
  ...
  // create a new iframe or object element (IE prints blank pdf's if we use iframe)
  if ((browser.isIE()||browser.isFirefox()) && print.params.type === 'pdf') {
    // create new element
    var pobj = document.getElementById(print.params.pid);
    pobj.innerHTML =
        '<object id="' + print.params.frameId + '" onreadystatechange="IePdfPrinter.pdfOnreadystatechange(this)"' +
        'width="0" height="0" type="application/pdf"' +
        'data="' + print.params.printable + '">' +
        '</object>';
...
PrintJS.prototype.pdf = function () {
  ...
  if (print.params.showModal && !browser.isIE()) {
    //在非IE浏览器下，并且需要pdf展示，那么使用promise技术
    ...
    if (browser.isIE()||browser.isFirefox()) {
      //print.printFrame.setAttribute('src', print.params.printable+"?#view=Fit&scrollbar=0&toolbar=0&navpanes=0");
    }else{
      // 将加载pdf的url指向到iframe
      print.printFrame.setAttribute('src', print.params.printable);
    }
...
/*  // append iframe element to document body
  ...
  if ((browser.isIE()||browser.isFirefox()) && print.params.type === 'pdf') {
    var pObj = document.getElementById(print.params.pid);
    pObj.appendChild(print.printFrame);
    var printJS = document.getElementById(print.params.frameId);
    printJS.onreadystatechange = finishPrintIePdfBySD();
  } else {
    // append iframe element to document body
    document.getElementsByTagName('body')[0].appendChild(print.printFrame);
  ...
  function finishPrintIePdfBySD() {
    console.dir(printJS);
    console.dir(printJS.readyState);
      if (printJS.readyState === 'complete'){
        setTimeout(function () {
          console.info('pdf 加载完成！');
          printJS.Print();
        }, 1000);
      }else{
        setTimeout(finishPrintIePdfBySD, 500);
      }
  }
...
window.IePdfPrinter = {
  pdfOnreadystatechange: function (obj) {
    if (obj.readyState === 4)
      setTimeout(function () {
        IePdfPrinter.printPdf(obj);
      }, 1000);
  },
  printPdf: function (idPdf) {
    idPdf.Print();
  }
};
