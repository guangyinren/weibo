		...
		success:function(data){
			if("200"==data.code){
				if(data.data.paperInvoiceCaution){
					layer.alert('您的纸质发票库存数量已不足'+data.data.paperInvoiceCautionValue+'张',{icon:2,title:['',true],skin:'newLayer'}, function(){
		        		layer.closeAll();
					});
				}
			}
...
//仅保存
function saveDzfp(){
	var actionUrl = basePath+"paperInvoice/savePaperInvoice"+suffix;
...
function saveMakeDzfp(){
	var actionUrl = basePath+"paperInvoice/saveAndMakePaperInvoice"+suffix;
	handleReqFun(actionUrl);
}
