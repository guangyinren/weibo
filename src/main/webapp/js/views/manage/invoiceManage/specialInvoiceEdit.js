	...
	$.ajax({
		...
		success:function(data){
			if (data.code == 200) {
	            ...
	            for (var arr = 1; arr <= item.proList.length; arr++) {
	            	var selectObj = $("#taxRate"+arr+"");
				    ...
				    for(var i=0;i<raxRates.length;i++){
		    			 ...
		    			 var taxTate = parseFloat(item.proList[arr-1].taxRate);
		    			 if(parseFloat(raxRates[i].taxRateValue) == taxTate){
		    				 selectObj.val(taxTate);
		    			 }
		            }  
//				$('#taxRate'+arr+' option:eq(0)').attr('selected','selected');
			     $("#productName"+arr+"").autocomplete({
			         ...
			         select: function(event,ui) {
			             $("#productName"+arr+"").val(ui.item.goodsName);
			             $("#taxClassificationCode"+arr+"").text(ui.item.mergeCoding);
			         },
	            ...
	            $("#productName"+arr+"").rules("add",{
	            	...
	            	remote: { //ajax调用后台查看手机号是否可用
	            		...
	            		type:"post",
	            		data:{
	            			taxClassificationCode:function(){return $("#taxClassificationCode"+(arr-1)+"").text();},
	            			taxClassificationName:function(){return $("#productName"+(arr-1)+"").val();}
	            		} 
	            	},
	            	messages:{
	            ...
	            $("#specificationModel"+arr+"").rules("add",{
	            	required:false,
       ...
       return;
		...
		layer.confirm("确定要删除本行吗？", {
			...
			ckbs.each(function(){
				 ...
				 if(idIndex != -1){
					 removeDisableOfProTr(id.substring(0,idIndex));
				 }else{
...
//仅保存
function saveDzfp(){
	var actionUrl = basePath+"paperInvoice/updateSpecialInvoice"+suffix;
	handleReqFun(actionUrl);
}

//保存开具发票
function saveMakeDzfp(){
	var actionUrl = basePath+"paperInvoice/updateAndMakeSpecialInvoice"+suffix;
	handleReqFun(actionUrl);
}
