	...
	$.ajax({
		...
		success:function(data){
			if (data.code == 200) {
	            ...
	            if(item.gmfNsrsbh == "" || item.gmfNsrsbh == null){
	            	$("#ttlx-gr").attr("checked","checked");
	            } else {
	            	$("#ttlx-dw").attr("checked","checked");
	            	$(".gmfNsrsbh-re").show();
	            }
...
function fillEditTaxCatData(item) {
        ...
        $("#fhr").val(item.fhr);
        $("#kpr").val(item.kpr);
        $("#sprMc").val(item.sprMc);
        $("#sprSjh").val(item.sprSjh);
        $("#sprYx").val(item.sprYx);
...
function addProTr(){
	...
	strs.push('<td class="column-invoice-taxes"><select id="taxRate'+index+'" name="taxRate'+index+'" onchange="countSePrice('+index+')"></td>');
...
//修改保存发票
function saveDzfp(){
	var actionUrl = basePath+"eleInvoice/updateEleInvoice"+suffix;
	handleReqFun(actionUrl);
}

//修改并开具发票
function saveMakeDzfp(){
	var actionUrl = basePath+"eleInvoice/updateAndMakeEleInvoice"+suffix;
	handleReqFun(actionUrl);
}
